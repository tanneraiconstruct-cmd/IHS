...
Executed via `superpowers:subagent-driven-development` in a worktree at `.worktrees/phase-4-ui/` on branch `feat/phase-4-ui` (created from `origin/main`).

**All 30 tasks complete.** Branch pushed; PR open at https://github.com/tanneraiconstruct-cmd/IHS/pull/10. Final state: 154 unit tests passing (24 files), 2 Playwright E2E tests passing, `npm run lint` / `typecheck` / `build` all clean.
...
| T12 | Gantt layout math | `f645cd7` |
| T13 | Gantt rendering (read-only) | `8bd79f9` |
| T14 | Problems badge + popover | `c33a09a` |
| T15 | List view | `613ec58` |
| T16 | Calendar view | `09372bf` |
| T17 | Lookahead view | `fb1633e` |
| T18 | Side panel feed | `78222ec` |
| T19 | Toast system | `9c9498b` |
| T20 | Mutation infrastructure | `7811ef2` |
| T21 | Edit-mode banner | `37429f6` |
| T22 | Drag-move Gantt bar | `2bbfa2a` |
| T23 | Drag-resize bar edge | `a4ce650` |
| T24 | Draw new dependency | `ccdf956` |
| T25 | Inline edit (name + duration) | `9a80273` |
| T26 | Delete activity / deactivate dep | `02511f8` |
| T27 | Comment composer | `ea7156a` |
| T28 | E2E: scheduler happy path | `c890917` |
| T29 | E2E: external user (RLS) | `e3b3b5b` |
| (fix) | Typecheck narrowing + gitignore | `eb3c926` |
| T30 | Push + open PR | (PR #10) |

### Outstanding before Phase 4 fully closes

1. **Manual §7 walkthrough** — three checks the automated suite can't do:
   - §7.5: drag-move / drag-resize / draw-dep / inline-edit / deactivate-dep / delete-activity in a real browser; confirm cascade dates land in Supabase + history rows written. T28's happy-path E2E only verifies plumbing (logins, view switching, edit-mode toggle) — drag interactions are explicitly deferred to follow-up.
   - §7.6: post an internal comment as `scheduler@ihs.test`, sign in as `tp-viewer@trade.test`, confirm it's hidden. The T29 E2E asserts "no internal chips visible" but doesn't first *write* one.
   - §7.7: open two browsers as the same scheduler, edit the same activity concurrently, confirm second editor gets the conflict toast.
2. **PR #10 review + merge.**

### Known limitations (deferred by the plan, not Phase 4 gaps)

- **Role-gating not implemented.** External `tp-viewer` still sees the "Edit mode" button. Writes are RLS-blocked, but the affordance is visible. Deferred to Phase 11 per the spec.
- **Cascade-write inconsistency window.** Cascaded rows are written without version checks (see Concurrency Model below). Acceptable for v1 single-team; Phase 6 fixes with realtime + server-side reconciliation.
- **Phase 3 server pipeline dormant.** `apply_schedule_edit` RPC + route handlers sit on disk unused.

### Errata applied during T1 – T11 (the literal plan code below has been patched)

The plan was written from training-data-era assumptions. The following corrections were folded into Tasks 1–11 during execution; the corresponding task sections below have been edited to show the corrected code.
...
### Errata observed during T12 – T30 (literal plan code NOT patched — fixes applied per-task)

The plan code in T12+ sections still reads as originally written. Future agents re-executing these sections should expect to apply the same fixes inline:

9. **T13 (GanttChart) and T16 (CalendarView) — unused-import lint.** Both have `import { isoAddDays, isoDiffDays }` from `./layout` (or `../Gantt/layout`) but reference only `isoDiffDays` (or neither). ESLint flags the unused ones; drop them from the import line.
10. **T17 (LookaheadView) — React Compiler `react-hooks/preserve-manual-memoization`.** The spec's `useMemo` with `lookahead` (= `lookaheads.find(...)`, a fresh object each render) as a dep was flagged by `eslint-plugin-react-hooks` because the compiler couldn't preserve the memoization. CalendarView, ListView, GanttChart use `useMemo` over stable deps and pass; T17's two memos were rewritten as IIFEs and the compiler memoizes them automatically. Same fix applies if a future task hits the rule.
11. **T20 (mutations.ts) — supabase-js v2 typing on update/insert results.** `res.data as DbActivity | null` and `data as DbDependency` need `as unknown as` (the T7 errata pattern). Also: indexing `current` and `vars.patch` as `Record<string, unknown>` requires `as unknown as Record<string, unknown>` because `DbActivity` has no string index signature.
12. **T20 + T11 — typecheck-only union-narrowing in test files.** `mutations.test.ts` accessed `result.kind` before narrowing `result.ok === false`; `utils.test.ts` accessed `.children` on `TreeRow` without narrowing `kind === "group"`. Both were latent — runtime tests passed but `tsc --noEmit` failed. Fixed in commit `eb3c926`.
13. **T26 — pointer-events on the Gantt SVG.** The dependency arrow needs to be double-clickable to toggle activation. The plan removed `pointer-events-none` from the SVG and added `pointerEvents="none"` to the `<marker>` so the arrowhead doesn't intercept clicks. Bars sit on top of the SVG as DOM siblings so they still receive clicks.
14. **T26 — fragment-wrap GanttBar branches.** The plan instructs adding the `<ContextMenu>` "as a sibling to the button". Because both the milestone and task branches `return` a single `<button>`, each branch must be wrapped in a fragment so the menu can render alongside.
15. **T28 — Playwright strict mode.** `page.getByText("Mobilize")` matches two DOM nodes in the List view (a `<span>` and a `<td>`). Append `.first()` to satisfy strict mode — the Calendar branch already does this.
16. **T29 — Playwright "internal" chip selector.** `page.locator("text=internal")` matches both the feed-item visibility chip AND the CommentComposer's visibility toggle button. Narrow to `page.locator("li").locator("text=internal")` so only the feed chip is asserted-absent.
