-- Phase 3 (Task 16 hotfix): re-apply apply_schedule_edit without referencing
-- the non-existent projects.updated_at column. The previous migration was
-- already pushed but its body errored at runtime on the project-patch UPDATE.
-- Idempotent forward-only fix via `create or replace function`.
